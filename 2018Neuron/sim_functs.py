...
def genPoissonTrain(Ensyn, bgErate, fgErate, duration, omegaUP, omegaDown, Insyn=0, bgIrate = 0, fgIrate = 0, random_seed=1):
    ...
    while (Etimes.shape[0]<2): # at least two spikes are required
        ...
        while (t_last < duration):
            ...
            PE =  br.input.spikegeneratorgroup.SpikeGeneratorGroup(Ensyn, Erate, dur_state * br.ms)
            ...
            if ((Insyn > 0)&(Irate > 0)) :
                ...
                PI =  br.input.spikegeneratorgroup.SpikeGeneratorGroup(Insyn, Irate, dur_state * br.ms)
...
def sim_nsynPoiss(Ensyn, Erate, bRate=0, minN=5, maxN=20):
    ...
    # Run
    fih =   h.FInitializeHandler(1, initSpikes)
    taxis, v, vD =   simulate(model, t_stop=data.TSTOP * 1000,
...
def SIM_currentSteps(iRange, bGround=False):
    ...
    for step in iRange:
        ...
        model.stim.amp = step
        taxis, v, vD =   simulate(model, t_stop=data.TSTOP * 1000, NMDA=data.NMDA, recDend=data.recordDend, i_recDend=data.locDendRec)
