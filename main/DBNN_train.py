...
def parse_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=0.08, help="Learning rate (default: 0.02)")
    parser.add_argument('--num_epochs', type=int, default=1500, help="Numer of epochs for training (default: 100)")
...
for epoch in range(num_epochs):
    ...
    total_time += epoch_time  # 累计总时间
    if (epoch + 1) % 10 == 0 or epoch == 0 or epoch == num_epochs - 1:
        # 转换为分钟并保留2位小数
        time_in_min = total_time / 60  
        print(f'Epoch [{epoch+1}/{num_epochs}], '
              f'Loss: {epoch_loss:.4f}, '
              f'Time: {time_in_min:.2f} minutes')



    # scheduler.step()

    # check the test loss every 100 epochs
    if (epoch + 1) % 100 == 0:
...
# save the model
# save_path = f'/home/mjy/project/bilinear_network/parameters/model_coarse_bs{batch_size}_lr{learning_rate}_epochs{num_epochs}_seed{manual_seed}.pth'
# loss_data_path = f'/home/mjy/project/bilinear_network/training_results/loss_data_bs{batch_size}_lr{learning_rate}_epochs{num_epochs}_seed{manual_seed}.json'
save_path = f'/home/mjy/cerebellum_mid_test/model_coarse_bs{batch_size}_lr{learning_rate}_epochs{num_epochs}_seed{manual_seed}.pth'
loss_data_path = f'/home/mjy/cerebellum_mid_testloss_data_bs{batch_size}_lr{learning_rate}_epochs{num_epochs}_seed{manual_seed}.json'
